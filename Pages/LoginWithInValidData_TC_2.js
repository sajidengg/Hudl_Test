...
// 1.The  function LoginWithInValidData() contians logic to validate the login test case using in-valid username and password 
...
let LoginWithInValidData = async function () {
    LoginData.Browser.forEach((browserType)=>{       
    try {
        describe("Check thew login test case with invalid data in: "+browserType, function () {
            before("Intizaling driver", async() => {
                await generic.LaunchServer("https://www.hudl.com/login",browserType);
            })
            it("Enter User Name", async () => {
                await generic.SendData(LoginObject.UserName, LoginData.InvalidUserName);
            })
            it("Enter Password", async() => {
                await generic.SendData(LoginObject.Password, LoginData.InvalidPassword);
            })
